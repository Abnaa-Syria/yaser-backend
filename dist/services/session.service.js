...
async function getMaxTrustedDevices() {
...
export const createUserSession = async (ctx) => {
    ...
    if (device) {
        ...
        if (isStudent && !device.isTrusted) {
            ...
            throw new AppError('This device is not trusted. Wait for admin approval or request a device replacement.', 403, {
        ...
        if (trustedDevices.length >= maxTrusted) {
            ...
            throw new AppError('Device limit reached. You can use up to 2 trusted devices. Request admin approval to replace one.', 403, {
                code: 'DEVICE_LIMIT',
                details: {
