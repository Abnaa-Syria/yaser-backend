 ...
 */
export function isCorruptedLocalizedText(value) {
    ...
    if (questionMarks >= 3 && !hasArabic)
        return true;
    return false;
}
export function cleanLocalizedString(value, fallback) {
    if (typeof value !== 'string')
        return fallback;
    const text = value.trim();
    if (!text || isCorruptedLocalizedText(text))
        return fallback;
    return text;
}
