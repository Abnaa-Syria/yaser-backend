...
function isHealthOrPublicSettings(req) {
    const path = req.originalUrl || req.url || '';
    if (/\/api\/health(?:\?|$)/.test(path))
        return true;
    // Site settings are polled by Header/Footer on every public page load.
    if ((req.method || 'GET').toUpperCase() === 'GET' && /\/public\/settings(?:\?|$)/.test(path))
        return true;
    return false;
}
...
export const apiLimiter = rateLimit({
    ...
    standardHeaders: true,
    legacyHeaders: false,
    skip: (req) => (isDev && req.ip === '::1') || isCertificateDownloadRequest(req) || isHealthOrPublicSettings(req),
});
