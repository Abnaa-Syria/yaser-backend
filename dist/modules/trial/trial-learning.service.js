...
async function requireTrialCourseAccess(courseId) {
...
function getExamStructure(exam) {
    if (Array.isArray(exam.examStructure) && exam.examStructure.length > 0)
        return exam.examStructure;
...
async function getCoveredTopics(exam) {
    const savedTopics = toStringList(exam.coveredTopics);
    if (savedTopics.length > 0)
        return savedTopics;
    if (exam.lessonId && exam.lesson?.title)
        return [exam.lesson.title];
    return [];
}
async function resolveTrialCourseIds(filters = {}) {
    const rows = await listActiveTrialCourses();
    const ids = rows.map((r) => r.courseId);
    if (filters.courseId) {
        if (!ids.includes(filters.courseId))
            return [];
...
async function assertTrialExamAccess(examId) {
...
async function shapeExamDetailsForTrial(exam, mySubmission) {
...
export async function listTrialExams(trialSessionId, filters = {}) {
    ...
    const where = { AND: [authorizedScope] };
    if (filters.unitId) {
    ...
    for (const s of submissions) {
        if (!submissionByExam.has(s.examId))
            submissionByExam.set(s.examId, s);
    }
...
export async function getTrialExamDetails(trialSessionId, examId) {
    const exam = await prisma.exam.findUnique({
        where: { id: examId },
        include: {
            course: { select: { id: true, title: true } },
    ...
    if (courseId) {
        const ok = await isCourseInActiveTrial(courseId);
        if (!ok)
            throw new AppError('This exam is not included in the free trial.', 403);
    }
...
export async function startTrialExam(trialSessionId, examId) {
...
function questionHasOption(question, optionId) {
...
export async function submitTrialExam(trialSessionId, examId, answers) {
    ...
    if (!submission)
        throw new AppError('Submission not found. Did you start the exam?', 404);
...
export async function getTrialExamResult(trialSessionId, submissionId) {
    ...
    if (result.trialSessionId !== trialSessionId)
        throw new AppError('Unauthorized.', 403);
    return result;
}
//# sourceMappingURL=trial-learning.service.js.map