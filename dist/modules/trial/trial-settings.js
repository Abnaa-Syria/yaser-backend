...
function asNumber(value, fallback, min = 1, max = 365) {
...
export async function loadTrialSettings() {
...
export function trialSettingsToKeyMap(settings) {
    const out = {};
    if (settings.enabled !== undefined)
        out.TRIAL_ENABLED = Boolean(settings.enabled);
    ...
    if (settings.popupEnabled !== undefined)
        out.TRIAL_POPUP_ENABLED = Boolean(settings.popupEnabled);
    if (settings.title !== undefined)
        out.TRIAL_TITLE = String(settings.title).slice(0, 200);
    if (settings.titleAr !== undefined)
        out.TRIAL_TITLE_AR = String(settings.titleAr).slice(0, 200);
    if (settings.subtitle !== undefined)
        out.TRIAL_SUBTITLE = String(settings.subtitle).slice(0, 500);
    if (settings.subtitleAr !== undefined)
        out.TRIAL_SUBTITLE_AR = String(settings.subtitleAr).slice(0, 500);
    if (settings.ctaLabel !== undefined)
        out.TRIAL_CTA_LABEL = String(settings.ctaLabel).slice(0, 120);
    if (settings.ctaLabelAr !== undefined)
        out.TRIAL_CTA_LABEL_AR = String(settings.ctaLabelAr).slice(0, 120);
...
export async function isCourseInActiveTrial(courseId) {
...
//# sourceMappingURL=trial-settings.js.map