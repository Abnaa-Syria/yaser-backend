...
export const listInstructorLiveSessions = async (instructorId) => {
...
export const createInstructorLiveSession = async (instructorId, data) => {
    ...
    if (data.courseId) {
        const course = await prisma.course.findFirst({
            where: { id: data.courseId, instructorId, deletedAt: null },
            select: { id: true, type: true },
        });
        if (!course)
            throw new AppError('Course not found or does not belong to you.', 404);
    ...
    // Allocate live commissions if course-linked
    if (data.courseId) {
        ...
        for (const p of paidCoursePayments) {
            try {
                ...
                console.error('Commission allocation failed for instructor live session', err);
            }
        }
    }
    return session;
};
export const updateInstructorLiveSession = async (id, instructorId, data) => {
    const existing = await prisma.liveSession.findFirst({
        where: { id, instructorId },
    });
    if (!existing)
        throw new AppError('Live session not found or does not belong to you.', 404);
    const updateData = {};
...
export const deleteInstructorLiveSession = async (id, instructorId) => {
    const existing = await prisma.liveSession.findFirst({
        where: { id, instructorId },
    });
    if (!existing)
...
//# sourceMappingURL=instructor-live-session.service.js.map