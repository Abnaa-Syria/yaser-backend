...
export const getCourseContent = catchAsync(async (req, res) => {
    const result = await studentCourseService.getCourseContent(req.user.id, req.params.id);
    successResponse({ res, data: result, message: 'Course content retrieved' });
});
export const getCourseExams = catchAsync(async (req, res) => {
    const result = await studentCourseService.getCourseExams(req.user.id, req.params.id);
