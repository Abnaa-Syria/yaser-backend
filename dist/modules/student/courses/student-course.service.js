...
function getCourseRecommendationCta(course) {
...
function shapeRecommendation(course, options) {
    const cta = options.isEnrolled
        ? {
            label: 'Continue Learning',
            action: 'CONTINUE',
            courseId: course.id,
            reason: options.progressPercentage && options.progressPercentage > 0
...
async function fetchPurchasableCourses(excludeCourseIds, take) {
...
async function fetchContinueLearningCourses(studentId, take) {
    const purchases = await prisma.coursePurchase.findMany({
    ...
    return purchases
        ...
        .filter((row) => {
        if (seen.has(row.course.id))
            return false;
        seen.add(row.course.id);
        return true;
    });
}
async function fetchPopularActiveCourses(take) {
 ...
 */
export const getRecommendedCourses = async (studentId, options = {}) => {
    ...
    recommendations = newCourses
        .map((course) => shapeRecommendation(course, {
        isEnrolled: false,
        strategy,
    }))
    ...
    if (recommendations.length < limit) {
        ...
        for (const row of continueRows) {
            if (recommendations.length >= limit || existingIds.has(row.course.id))
                continue;
            recommendations.push(shapeRecommendation(row.course, {
                isEnrolled: true,
                strategy,
                progressPercentage: row.progressPercentage,
            }));
    ...
    if (recommendations.length < limit) {
        ...
        for (const course of popularCourses) {
            if (recommendations.length >= limit || existingIds.has(course.id))
                continue;
            recommendations.push(shapeRecommendation(course, {
 ...
 */
export const getCourseContent = async (studentId, courseId) => {
 ...
 */
export const getCourseExams = async (studentId, courseId) => {
    await requireCourseAccess(studentId, courseId);
