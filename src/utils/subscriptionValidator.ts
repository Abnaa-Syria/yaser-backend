...
function isAccessActive(expiresAt: Date | null | undefined): boolean {
...
export const hasActiveSubscription = async (studentId: string): Promise<boolean> => {
  const now = new Date();
...
async function courseEligibleForSubscriptionAccess(courseId: string): Promise<boolean> {
...
export const hasCourseAccess = async (studentId: string, courseId: string): Promise<boolean> => {
  if (await hasLifetimePurchase(studentId, courseId)) return true;
...
export const requireCourseAccess = async (studentId: string, courseId: string) => {
  ...
  if (!allowed) {
    throw new AppError(
      'Access denied. Purchase this course or activate a platform subscription for access.',
      403
    );
  }
};
