...
import {
  countInstructorUpcomingSessions,
  fetchInstructorUpcomingSessions,
} from './instructor-upcoming-sessions.js';
...
export const getInstructorOverview = async (instructorId: string) => {
  ...
  const earningsTrend = buckets.map((b) => {
    ...
    for (const tx of earningTxns) {
      if (tx.createdAt >= b.start && tx.createdAt <= b.end) total += tx.amount;
    }
