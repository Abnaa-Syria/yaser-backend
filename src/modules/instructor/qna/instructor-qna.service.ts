...
async function assertInstructorOwnsCourse(instructorId: string, courseId: string) {
...
export const replyToQuestion = async (instructorId: string, questionId: string, body: string) => {
...
export const toggleResolve = async (instructorId: string, questionId: string) => {
  const question = await prisma.lessonQuestion.findUnique({
    where: { id: questionId },
    include: {
...
export const listInstructorQuestions = async (
  instructorId: string,
  query: { courseId?: string; lessonId?: string; page?: number; limit?: number; resolved?: string }
) => {
