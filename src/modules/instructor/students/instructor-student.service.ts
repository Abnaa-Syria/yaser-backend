...
export const listInstructorStudents = async (
  instructorId: string,
  query: { courseId?: string; search?: string; page?: number; limit?: number }
) => {
  const pageNum = Number(query.page) || 1;
  const limitNum = Math.min(Number(query.limit) || 20, 100);
  const skip = (pageNum - 1) * limitNum;

  const and: Record<string, unknown>[] = [{ course: { instructorId } }];
 ...
 */
export const getStudentPerformanceForInstructor = async (instructorId: string, studentId: string) => {
  ...
  const examInScopedCourses = (exam: {
    ...
    unit: { courseId: string } | null;
    lesson: { section: { unit: { courseId: string } } } | null;
  }): boolean => {
    if (exam.courseId && courseIdSet.has(exam.courseId)) return true;
    if (exam.unit?.courseId && courseIdSet.has(exam.unit.courseId)) return true;
    const lessonCourseId = exam.lesson?.section?.unit?.courseId;
    if (lessonCourseId && courseIdSet.has(lessonCourseId)) return true;
    return false;
  };
