...
async function assertInstructorOwnsCourse(courseId: string, instructorId: string) {
...
export const getEnrolledStudents = async (instructorId: string, courseId: string) => {
...
export const getAllEnrolledStudents = async (instructorId: string) => {
  const purchases = await prisma.coursePurchase.findMany({
    where: {
      course: {
        instructorId,
