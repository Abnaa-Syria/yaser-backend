...
const instructorExamScope = (instructorId: string): Prisma.ExamWhereInput => ({
  OR: [
    { course: { instructorId } },
...
async function resolveExamCourseId(exam: { courseId: string | null; unitId: string | null; lessonId: string | null }) {
...
const verifyExamOwnership = async (instructorId: string, examId: string) => {
  const exam = await prisma.exam.findUnique({ where: { id: examId } });
  if (!exam) throw new AppError('Exam not found.', 404);
  const courseId = await resolveExamCourseId(exam);
  if (!courseId) throw new AppError('Exam not found.', 404);
  await verifyCourseOwnership(instructorId, courseId);
  return exam;
};

export const listInstructorExams = async (instructorId: string, query: { search?: string }) => {
  const andConditions: Prisma.ExamWhereInput[] = [instructorExamScope(instructorId)];
...
export const getExamDetailForInstructor = async (instructorId: string, examId: string) => {
...
export const getCourseStructureForInstructor = async (instructorId: string, courseId: string) => {
  await verifyCourseOwnership(instructorId, courseId);
...
function flattenLessonsFromUnits(
  units: Array<{
    id: string;
    title: string;
    sections: Array<{ id: string; title: string; lessons: Array<{ id: string; title: string; order: number }> }>;
  }>
) {
  const lessons: Array<{
    id: string;
...
export const createExam = async (instructorId: string, data: any) => {
  let unitId: string | null = data.unitId ?? null;
  let lessonId: string | null = data.lessonId ?? null;
  ...
  let type: ExamType = ExamType.STANDALONE;
  if (lessonId) type = ExamType.LESSON;
  else if (unitId) type = ExamType.UNIT;
...
export const updateExam = async (instructorId: string, examId: string, data: any) => {
  await verifyExamOwnership(instructorId, examId);
...
export const deleteExam = async (instructorId: string, examId: string) => {
  await verifyExamOwnership(instructorId, examId);
  await prisma.exam.delete({ where: { id: examId } });
  return { id: examId, deleted: true };
};

export const addQuestion = async (instructorId: string, examId: string, data: any) => {
  await verifyExamOwnership(instructorId, examId);
...
export const updateQuestion = async (instructorId: string, examId: string, questionId: string, data: any) => {
  await verifyExamOwnership(instructorId, examId);
...
export const removeQuestion = async (instructorId: string, examId: string, questionId: string) => {
  await verifyExamOwnership(instructorId, examId);
...
export const getExamSubmissions = async (instructorId: string, examId: string) => {
  await verifyExamOwnership(instructorId, examId);
