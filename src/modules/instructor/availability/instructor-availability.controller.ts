...
export const listAvailabilityHandler = catchAsync(async (req: Request, res: Response) => {
...
export const deleteAvailabilityHandler = catchAsync(async (req: Request, res: Response) => {
...
export const addAvailabilityHandler = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await availabilityService.addAvailability(
    req.user.id,
    new Date(startTime),
    new Date(endTime),
    parsedPrice
...
export const updateAvailabilityPriceHandler = catchAsync(async (req: Request, res: Response) => {
  const parsedPrice = req.body?.price != null ? Number(req.body.price) : NaN;
  const result = await availabilityService.updateAvailabilitySlotPrice(
    req.user.id,
