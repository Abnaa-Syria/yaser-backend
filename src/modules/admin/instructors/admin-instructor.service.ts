...
import {
  ...
  updateAvailabilitySlotPrice,
} from '../../instructor/availability/instructor-availability.service.js';
...
export const getAllInstructors = async (query: any) => {
  ...
  const where: any = courseOwnerRoleFilter();
...
export const getInstructorById = async (id: string) => {
  ...
  const stats = {
    ...
    totalStudents: instructor.coursesInstructed.reduce((acc: number, course) => acc + course._count.purchases, 0),
 ...
 */
export const getInstructorPerformanceForAdmin = async (instructorId: string) => {
  const exists = await prisma.user.findFirst({
    where: { id: instructorId, ...courseOwnerRoleFilter() },
    select: { id: true },
  });
  if (!exists) throw new AppError('Instructor not found.', 404);
 ...
 */
export const getInstructorAvailabilityForAdmin = async (instructorId: string) => {
  const exists = await prisma.user.findFirst({
    where: { id: instructorId, ...courseOwnerRoleFilter() },
    select: { id: true },
  });
...
export const createInstructorAvailabilityForAdmin = async (
  instructorId: string,
  data: { startTime: string | Date; endTime: string | Date; price: number }
) => {
  const exists = await prisma.user.findFirst({
    where: { id: instructorId, ...courseOwnerRoleFilter() },
    select: { id: true },
...
export const deleteInstructorAvailabilityForAdmin = async (instructorId: string, slotId: string) => {
  return deleteAvailabilitySlot(instructorId, slotId);
};

export const updateInstructorAvailabilityPriceForAdmin = async (
  instructorId: string,
  slotId: string,
  price: number
) => {
