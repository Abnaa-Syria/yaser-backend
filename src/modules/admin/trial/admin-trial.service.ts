...
import {
  loadTrialSettings,
  trialSettingsToKeyMap,
  type TrialSettings,
} from '../../trial/trial-settings.js';
...
export async function updateAdminTrialSettings(partial: Partial<TrialSettings>) {
...
export async function replaceTrialCourses(
  items: Array<{ courseId: string; displayOrder?: number; isActive?: boolean }>
) {
...
export async function revokeTrialSession(sessionId: string, reason?: string) {
...
export async function restoreTrialSession(sessionId: string) {
  const session = await prisma.trialSession.findUnique({ where: { id: sessionId } });
  if (!session) throw new AppError('Trial session not found.', 404);
  if (!session.revokedAt) {
