 ...
 */
export const getAllEnrollments = async (raw: ListOptions) => {
  const pageNum = Number(raw.page) || 1;
  const limitNum = Math.min(Number(raw.limit) || 10, 100);
  const skip = (pageNum - 1) * limitNum;

  const and: Record<string, unknown>[] = [];
  if (raw.courseId) and.push({ courseId: raw.courseId });
  if (raw.studentId) and.push({ studentId: raw.studentId });
  if (raw.status === 'completed') and.push({ isCompleted: true });
  if (raw.status === 'active') and.push({ isCompleted: false });
  if (raw.dateFrom || raw.dateTo) {
    const purchasedAt: { gte?: Date; lte?: Date } = {};
    if (raw.dateFrom) purchasedAt.gte = new Date(raw.dateFrom);
    if (raw.dateTo) {
  ...
  let paymentSums = new Map<string, number>();
  if (keys.length > 0) {
    ...
    paymentSums = new Map<string, number>();
  ...
  return {
    enrollments: purchases.map((p) => {
      ...
      const statusLabel: 'completed' | 'active' = p.isCompleted ? 'completed' : 'active';
 ...
 */
export const updateEnrollmentExpiry = async (id: string, expiresAt: string | null) => {
 ...
 */
export const revokeEnrollment = async (id: string) => {
  const existing = await prisma.coursePurchase.findUnique({ where: { id } });
  if (!existing) throw new AppError('Enrollment not found.', 404);
