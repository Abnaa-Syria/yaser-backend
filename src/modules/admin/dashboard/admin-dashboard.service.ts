...
function monthBuckets(count: number) {
  const now = new Date();
  const buckets: { monthKey: string; label: string; start: Date; end: Date }[] = [];
...
function paymentEffectiveDate(p: { paidAt: Date | null; createdAt: Date }) {
...
export const getOverview = async () => {
  ...
  const [
    ...
    prisma.$queryRaw<Array<{ courseId: string; title: string; enrollmentCount: bigint }>>(
      Prisma.sql`
  ...
  const revenueTrend = buckets.map((b) => {
    ...
    for (const p of paidInRange) {
      ...
      if (at >= b.start && at <= b.end) total += p.amount;
    }
