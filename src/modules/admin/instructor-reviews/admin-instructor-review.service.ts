...
async function resolveInstructorId(instructorId?: string | null) {
  if (instructorId) return instructorId;
  const platformId = await getPlatformInstructorId();
  if (!platformId) throw new AppError('Platform instructor is not configured.', 400);
  return platformId;
}

async function refreshAverageRating(instructorId: string) {
...
export const createInstructorReview = async (
  data: {
    instructorId?: string;
    displayName: string;
    rating: number;
  ...
  if (data.studentId) {
    ...
    if (existing) throw new AppError('This student already reviewed this doctor.', 409);
  }
...
export const updateInstructorReview = async (
  reviewId: string,
  data: {
    displayName?: string;
    rating?: number;
...
export const deleteInstructorReview = async (reviewId: string, actorId?: string) => {
  const existing = await prisma.instructorReview.findUnique({ where: { id: reviewId } });
  if (!existing) throw new AppError('Review not found.', 404);
