...
export const updateCommission = async (instructorId: string, commissionRate: number) => {
...
export const getPayoutRequests = async (query: any) => {
...
export const processPayout = async (
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!payout) throw new AppError('Payout request not found', 404);
    if (payout.status === 'PAID') throw new AppError('Payout already processed as PAID', 400);
    if (payout.status === 'REJECTED') throw new AppError('Payout already rejected', 400);

    const wallet = payout.instructor.wallet;
    if (!wallet) throw new AppError('Instructor wallet not found', 404);
