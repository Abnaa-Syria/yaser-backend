 ...
 */
export const getAllPayments = async (query: { status?: string; studentId?: string }) => {
...
async function fulfillPaidPaymentTx(
  ...
  payment: {
    ...
    coursePackagePricingTierId?: string | null;
  },
  ...
  if (payment.courseId && !payment.availabilityId) {
    ...
    if (payment.pricingTierId) {
      ...
      if (tier) {
        expiresAt = calculateAccessExpiresAt(
          accessStartsAt,
          durationDaysFromParts(tier.durationDays, tier.durationValue, tier.durationUnit)
        );
      }
    }
...
async function calculatePaymentAccessExpiresAtTx(
  ...
  if (payment.pricingTierId) {
    ...
    if (tier) {
      return calculateAccessExpiresAt(
        accessStartsAt,
        durationDaysFromParts(tier.durationDays, tier.durationValue, tier.durationUnit)
      );
    }
...
export const approvePayment = async (
  ...
  const productTitle =
    payment.course?.title || payment.coursePackage?.title || 'your purchase';
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await createNotification(
      ...
      rejectionReason?.trim()
        ...
        : 'Your recent payment attempt was rejected. Please contact support.',
      'GENERAL',
      tx
    );
  });
