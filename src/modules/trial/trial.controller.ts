...
export const me = catchAsync(async (req: Request, res: Response) => {
  const fingerprint =
    (typeof req.get('x-device-fingerprint') === 'string' && req.get('x-device-fingerprint')) ||
...
export const listCourses = catchAsync(async (_req: Request, res: Response) => {
  const courses = await trialService.getTrialCourses();
  res.status(200).json({ success: true, data: { courses } });
});

export const courseUnits = catchAsync(async (req: Request, res: Response) => {
  const units = await trialService.getTrialCourseContent(String(req.params.id));
  res.status(200).json({ success: true, data: units });
});

export const lessonPlayback = catchAsync(async (req: Request, res: Response) => {
...
export const recordings = catchAsync(async (_req: Request, res: Response) => {
  const items = await trialService.getTrialRecordings();
  res.status(200).json({ success: true, data: { recordings: items } });
});

export const flashcards = catchAsync(async (req: Request, res: Response) => {
...
export const examResults = catchAsync(async (req: Request, res: Response) => {
