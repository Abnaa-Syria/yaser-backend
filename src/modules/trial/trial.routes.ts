...
publicTrialRouter.post(
  '/start',
  trialStartLimiter,
  validate(trialValidation.startTrialSchema),
...
trialRouter.get(
  '/exams/:id',
  validate(trialValidation.examIdParamSchema),
  trialController.getExam
);
trialRouter.post(
  '/exams/:id/start',
  validate(trialValidation.examIdParamSchema),
  trialController.startExam
);
trialRouter.post(
  '/exams/:id/submit',
  validate(trialValidation.submitTrialExamSchema),
  trialController.submitExam
);
trialRouter.get(
  '/exams/:id/results/:submissionId',
  validate(trialValidation.examResultParamSchema),
  trialController.examResults
);
trialRouter.get(
  '/courses/:id/units',
  validate(trialValidation.courseIdParamSchema),
  trialController.courseUnits
);
trialRouter.get(
  '/lessons/:lessonId/playback',
  validate(trialValidation.lessonIdParamSchema),
  trialController.lessonPlayback
);
