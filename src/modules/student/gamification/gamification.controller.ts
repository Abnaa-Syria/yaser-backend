...
export const getCurrentChallenge = catchAsync(async (req: Request, res: Response) => {
  const data = await gamificationService.getCurrentChallenge(req.user!.id);
  successResponse({ res, data });
});

export const patchPrivacy = catchAsync(async (req: Request, res: Response) => {
...
export const postFlashcardSession = catchAsync(async (req: Request, res: Response) => {
