import type { ChallengeGoalType, Prisma, XpSourceType } from '@prisma/client';
...
export function xpToNextLevel(totalXp: number): { level: number; current: number; next: number; progress: number } {
...
function parseDateOnly(value: Date | string | null | undefined): string | null {
...
export type AwardXpResult = {
  ...
  newBadges: Array<{ key: string; titleEn: string; titleAr: string }>;
  challengeCompleted?: boolean;
};

async function ensureProfile(userId: string, tx: Prisma.TransactionClient = prisma) {
...
function applyStreak(profile: { currentStreak: number; longestStreak: number; lastActiveDate: Date | null }, today: string) {
...
type BadgeRule =
  ...
  | { type: 'EXAM_SCORE'; minPercent: number }
  | { type: 'CERTIFICATES'; count: number }
  | { type: 'TOTAL_XP'; amount: number }
  | { type: 'CHALLENGE_COMPLETED'; count: number }
  | { type: 'FIRST_LESSON' };

async function evaluateBadges(userId: string, tx: Prisma.TransactionClient) {
  ...
  const unlocked: Array<{ key: string; titleEn: string; titleAr: string; xpReward: number; id: string }> = [];

  for (const def of defs) {
    if (ownedIds.has(def.id)) continue;
    const rule = (def.rule || {}) as BadgeRule;
    let ok = false;
    if (rule.type === 'FIRST_LESSON') ok = lessonCount >= 1;
    if (rule.type === 'LESSONS_COMPLETED') ok = lessonCount >= (rule.count || 0);
    if (rule.type === 'STREAK') ok = (profile?.currentStreak || 0) >= (rule.days || 0) || (profile?.longestStreak || 0) >= (rule.days || 0);
    if (rule.type === 'CERTIFICATES') ok = certCount >= (rule.count || 0);
    if (rule.type === 'TOTAL_XP') ok = (profile?.totalXp || 0) >= (rule.amount || 0);
    if (rule.type === 'CHALLENGE_COMPLETED') ok = challengeDone >= (rule.count || 0);
...
async function bumpWeeklyChallenge(
  userId: string,
  event: { sourceType: XpSourceType; amount: number; meta?: Record<string, unknown> },
  ...
  let delta = 0;
  if (challenge.goalType === 'COMPLETE_LESSONS' && event.sourceType === 'LESSON') delta = 1;
  if (challenge.goalType === 'PASS_EXAMS' && event.sourceType === 'EXAM' && event.meta?.passed === true) delta = 1;
  if (challenge.goalType === 'EARN_XP') delta = Math.max(0, event.amount);
  if (delta <= 0) return { completed: false };
...
export async function ensureCurrentWeeklyChallenge(tx: Prisma.TransactionClient = prisma) {
  const now = new Date();
  ...
  return tx.weeklyChallenge.create({
    data: {
      ...
      goalType: 'COMPLETE_LESSONS' as ChallengeGoalType,
      goalTarget: 5,
...
export async function awardXp(input: AwardXpInput): Promise<AwardXpResult> {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      try {
        ...
        if (err?.code === 'P2002') {
          ...
          return {
            ...
            newBadges: [] as AwardXpResult['newBadges'],
          };
        }
        throw err;
      }
      ...
      const challenge = await bumpWeeklyChallenge(
        input.userId,
        { sourceType: input.sourceType, amount, meta: input.meta },
        tx
      );
...
export async function awardLessonXp(userId: string, lessonId: string, courseId?: string) {
  return awardXp({
    userId,
    amount: XP.LESSON,
...
export async function awardExamXp(
  userId: string,
  submissionId: string,
  opts: { score: number; passingScore: number; totalPoints: number; courseId?: string; passed: boolean }
) {
...
export async function awardStudyPlanItemXp(userId: string, itemId: string) {
  return awardXp({
    userId,
...
export async function awardCertificateXp(userId: string, certificateId: string, courseId?: string) {
  return awardXp({
    userId,
...
export async function awardFlashcardSessionXp(userId: string, sessionKey: string) {
  const today = utcDateString();
  ...
  if (already >= XP.FLASHCARD_DAILY_CAP) {
    return { awarded: false, amount: 0, totalXp: 0, level: 1, currentStreak: 0, newBadges: [] as AwardXpResult['newBadges'] };
  }
...
export function publicDisplayName(fullName: string | null | undefined, optOut: boolean) {
  if (optOut) return 'Anonymous';
...
export async function getLeaderboard(opts: {
  ...
  const where: Prisma.XpLedgerWhereInput = {
    ...(opts.period === 'week' ? { createdAt: { gte: weekStart } } : {}),
  };

  let grouped: Array<{ userId: string; _sum: { amount: number | null } }>;
  ...
  if (opts.scope === 'course' && opts.courseId) {
    ...
    for (const row of rows) {
      ...
      if (meta.courseId !== opts.courseId) continue;
...
export async function listBadgesCatalog(userId: string) {
...
export async function updateDisplayOptOut(userId: string, optOut: boolean) {
  await ensureProfile(userId);
