...
router.post(
  ...
  restrictToStudent,
  validate(financialValidation.courseCheckoutSchema),
  financialController.courseCheckout
);
router.post(
  '/checkout/package/:packageId',
  restrictToStudent,
  validate(financialValidation.packageCheckoutSchema),
  financialController.packageCheckout
);
router.post(
  '/checkout/private/:availabilityId',
  restrictToStudent,
  requireFeature('privateBooking'),
  validate(financialValidation.privateCheckoutSchema),
