...
async function findStudentByCredentials(identifier: string, password: string) {
  ...
  if (!ok) throw new AppError('Invalid email/username or password.', 401);
...
export async function requestDeviceReplacement(input: {
  ...
  if (!oldDevice) throw new AppError('Trusted device not found.', 404);
...
export async function listDeviceReplacementRequests(status?: string) {
  const where =
...
export async function approveDeviceReplacement(id: string, adminId: string, note?: string) {
...
export async function rejectDeviceReplacement(id: string, adminId: string, note?: string) {
  const req = await prisma.deviceReplacementRequest.findUnique({ where: { id } });
  if (!req) throw new AppError('Request not found.', 404);
  if (req.status !== 'PENDING') throw new AppError('Request is not pending.', 400);
