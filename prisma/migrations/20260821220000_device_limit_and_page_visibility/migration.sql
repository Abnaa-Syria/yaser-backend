...
-- Compatible with MariaDB (no CAST(... AS JSON)).
...
SELECT UUID(), 'MAX_TRUSTED_DEVICES', 2, CURRENT_TIMESTAMP(3)
WHERE NOT EXISTS (SELECT 1 FROM `platform_settings` WHERE `key` = 'MAX_TRUSTED_DEVICES');

UPDATE `platform_settings`
SET `value` = 2, `updatedAt` = CURRENT_TIMESTAMP(3)
WHERE `key` = 'MAX_TRUSTED_DEVICES';

INSERT INTO `platform_settings` (`id`, `key`, `value`, `updatedAt`)
SELECT UUID(), 'PUBLIC_PAGE_VISIBILITY',
  '{"home":true,"explore":true,"packages":false,"instructors":true,"events":true,"about":true,"contact":true,"faq":true,"blogs":true,"library":true,"teach":true,"guide":true,"terms":true,"privacy":true,"refund":true}',
  CURRENT_TIMESTAMP(3)
